...
mod rukinia_cmd {
    ...
    fn test_rukinia_cmd_success() {
        let result = RukiniaCmd::new(vec!["ls".to_string()], SyntaxForTrait::Arguments(Vec::new()));
        assert!(result.is_ok());
    ...
    fn test_rukinia_cmd_not_success() {
        let result = RukiniaCmd::new(vec!["ls".to_string()], SyntaxForTrait::Arguments(vec![Syntax::Not]));
        assert!(result.is_ok());
        let rukinia_cmd  = result.unwrap();
    ...
    fn test_rukinia_cmd_fail() {
        let result = RukiniaCmd::new(vec!["not_an_executable_in_the_system".to_string()], SyntaxForTrait::Arguments(Vec::new()));
        assert!(result.is_ok());
        let rukinia_cmd  = result.unwrap();
        assert!(rukinia_cmd.get_result().result_type == RukiniaResultType::TestFail);
        assert!(rukinia_cmd.display_format() == "Running \"not_an_executable_in_the_system\" is successful");
    }
